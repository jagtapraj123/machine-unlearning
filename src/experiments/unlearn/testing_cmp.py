...
if __name__ == "__main__":
    ...
    parser.add_argument("retrained_model_path", type=str, help='Path to saved retrained teacher model')
    ...
    print("Re-Trained Teacher Path: {}".format(retrained_model_path))
    ...
    ret_model = ResNet18(20 if coarse_labels else 100, pretrained=True)
    # print(ret_model)
    # summary(ret_model, (3, 32, 32), device='cpu')

    ret_model.load_state_dict(torch.load(retrained_model_path, map_location='cpu')["model_state_dict"])
    ...
    for score_set in score_sets:
        ...
        with torch.no_grad():
            ...
            for x_test, y_test, idx in tqdm(test_loader):
                ...
                y_target = ret_model(x)

                # loss = torch.nn.functional.kl_div(y_pred, y_target)
                ys.append(torch.nn.functional.log_softmax(y_target, dim=1).cpu().detach())
                y_preds.append(torch.nn.functional.log_softmax(y_pred, dim=1).cpu().detach())

                # ys += list(torch.argmax(y_target, dim=1).cpu().detach().numpy())
        ...
        print(ys.shape, flush=True)
        print(y_preds.shape, flush=True)

        kl_div = torch.nn.functional.kl_div(y_preds, ys, log_target=True)

        ys = list(torch.argmax(ys, dim=1).cpu().detach().numpy())
        y_preds = list(torch.argmax(y_preds, dim=1).cpu().detach().numpy())
        ...
        if isinstance(score_functions, list) and len(score_functions) > 0:
            ...
            print(
                "Testing {}, Set:{}, KLDiv:{}, Validation Scores:{}".format(
                    test_name, score_set["name"], kl_div.item(), validation_scores
                ),
                flush=True,
            )
