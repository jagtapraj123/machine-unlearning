...
if __name__ == "__main__":
    ...
    trainer = UNSIR(
        ...
        cuda_num=cuda_num,
        num_classes=20 if coarse_labels else 100,
        pin_memory=True
    )
    ...
    loss_training_log = trainer.unsir_train(
        unlearning_class=0,
    ...
    impair_training_log, impair_validation_log = trainer.train(
        unlearning_class=0,
    ...
    repair_training_log, repair_validation_log = trainer.train(
        unlearning_class=0,
    ...
    with open(
        os.path.join("logs/logfiles", trainer.name, "impair_training_log.pkl"), "wb"
    ) as f:
        pickle.dump(impair_training_log, f)

    with open(
        os.path.join("logs/logfiles", trainer.name, "impair_validation_log.pkl"), "wb"
    ) as f:
        pickle.dump(impair_validation_log, f)

    with open(
        os.path.join("logs/logfiles", trainer.name, "repair_training_log.pkl"), "wb"
    ) as f:
        pickle.dump(repair_training_log, f)

    with open(
        os.path.join("logs/logfiles", trainer.name, "repair_validation_log.pkl"), "wb"
    ) as f:
        pickle.dump(repair_validation_log, f)
