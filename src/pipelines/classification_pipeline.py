    ...
    def train(
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for x_train, y_train, idx in pbar:
                ...
                if epoch == 1:
                    print("Using {} samples".format(sum(s1_selector)))
                x = x[s1_selector]
                ...
                for i in range(len(idx)):
                    grad = torch.autograd.grad(loss_func_with_grad(y_pred[i], y_truth[i]), self.model.final.parameters(), create_graph=True)
